from code_gen.instruction import Instruction
...
class CodeGenerator:
    ...
    def _handle_gate_expression(self, node, parent_identifier):
        ...
        # Handle double gate expressions
            ...
            if self.optimize:
                return self._handle_optimized_double_gate_expression(
                    gate_type, children[1], children[2], parent_identifier
                )
            return self._handle_double_gate_expression(
                gate_type, children[1], children[2], parent_identifier
            )
        else:
            raise Exception(
                f'Invalid number of arguments ({len(children)-1}) passed to gate {gate_type.upper()}'
            )
    ...
    def _handle_optimized_double_gate_expression(
        self, gate_type, arg1_node, arg2_node, parent_identifier
    ):
        arg1_id = None
        arg2_id = None
        if str(arg1_node.label) != 'GateExpression':
    ...
    def _handle_declaration(self, node):
        ...
        # 2. Wire declaration
            ...
            temp_reg = self._handle_gate_expression(
                val_node, identifier
