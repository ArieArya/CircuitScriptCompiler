...
# States
START_STATE = 'START'
# Combined since keyword can be a subset of identifier
IDENTIFIER_KEYWORD_STATE = 'IDENTIFIER_KEYWORD'
...
class Token:
    ...
    def __repr__(self):
        return f'{self.type: <16} (Value = {repr(self.value)})'
    ...
    def advance(self):
        ...
        # State transitions
            ...
            if self.cur_state.name == IDENTIFIER_KEYWORD_STATE:
                ...
                self.cur_state.name == FIRST_PUNCTUATION_STATE
                or self.cur_state.name == SECOND_PUNCTUATION_STATE
            ):
                self.tokens.append(Token('PUNCTUATION', self.cur_string))
            elif (
                self.cur_state.name == FIRST_EQUAL_STATE
                or self.cur_state.name == SECOND_EQUAL_STATE
            ):
